...
object Serialization {
    ...
    private fun Output.writeWaitForFundingSigned(o: WaitForFundingSigned) = o.run {
        ...
        writeNullable(liquidityLease) { writeLiquidityLease(it) }
    ...
    private fun Output.writeNormal(o: Normal) = o.run {
        ...
        when (spliceStatus) {
            is SpliceStatus.WaitingForSigs -> {
                ...
                writeNullable(spliceStatus.liquidityLease) { writeLiquidityLease(it) }
    ...
    private fun Output.writeLiquidityLeaseFees(fees: LiquidityAds.LeaseFees) {
    ...
    private fun Output.writeLiquidityLease(lease: LiquidityAds.Lease) {
        write(0x00) // discriminator, in case we change the lease format in the future
        writeNumber(lease.amount.toLong())
        writeLiquidityLeaseFees(lease.fees)
        when (lease.paymentDetails) {
            is LiquidityAds.PaymentDetails.FromChannelBalance -> write(0x00)
            is LiquidityAds.PaymentDetails.FromFutureHtlc -> {
                write(0x80)
                writeCollection(lease.paymentDetails.paymentHashes) { writeByteVector32(it) }
            }
        ...
        when (lease.witness) {
            is LiquidityAds.FundingLeaseWitness.Basic -> {
                write(0x00)
                writeDelimited(lease.witness.fundingScript.toByteArray())
            }
        }
    }
    ...
    private fun Output.writeChannelParams(o: ChannelParams) = o.run {
        ...
        localParams.run {
            ...
            val payCommitTxFeesFlag = if (payCommitTxFees) 2 else 0
