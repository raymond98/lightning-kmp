...
object Deserialization {
    ...
    private fun Input.readNormal(): Normal = Normal(
        ...
        spliceStatus = when (val discriminator = read()) {
            ...
            0x01 -> SpliceStatus.WaitingForSigs(readInteractiveTxSigningSession(), readNullable { readLiquidityLease() }, readCollection { readChannelOrigin() }.toList())
    ...
    private fun Input.readLiquidityLeaseFees(): LiquidityAds.LeaseFees = LiquidityAds.LeaseFees(miningFee = readNumber().sat, serviceFee = readNumber().sat)

    private fun Input.readLiquidityLease(): LiquidityAds.Lease = when (val discriminator = read()) {
        0x00 -> LiquidityAds.Lease(
            amount = readNumber().sat,
            fees = readLiquidityLeaseFees(),
            ...
            witness = when (val witnessDiscriminator = read()) {
                0x00 -> LiquidityAds.FundingLeaseWitness.Basic(readDelimitedByteArray().byteVector())
                else -> error("unknown discriminator $witnessDiscriminator for class ${LiquidityAds.FundingLeaseWitness::class}")
            }
        )
        else -> error("unknown discriminator $discriminator for class ${LiquidityAds.Lease::class}")
    }
