...
data class OpenDualFundedChannel(
    ...
    val requestFunds: LiquidityAds.RequestFunds? get() = tlvStream.get<ChannelTlv.RequestFundingTlv>()?.request
...
data class SpliceInit(
    ...
    val requestFunds: LiquidityAds.RequestFunds? = tlvStream.get<ChannelTlv.RequestFundingTlv>()?.request
    val pushAmount: MilliSatoshi = tlvStream.get<ChannelTlv.PushAmountTlv>()?.amount ?: 0.msat

    constructor(channelId: ByteVector32, fundingContribution: Satoshi, pushAmount: MilliSatoshi, feerate: FeeratePerKw, lockTime: Long, fundingPubkey: PublicKey, requestFunds: LiquidityAds.RequestFunds?) : this(
        channelId,
        fundingContribution,
        feerate,
        lockTime,
        ...
        TlvStream(
            setOfNotNull(
                ...
                requestFunds?.let { ChannelTlv.RequestFundingTlv(it) },
            )
        )
    )
