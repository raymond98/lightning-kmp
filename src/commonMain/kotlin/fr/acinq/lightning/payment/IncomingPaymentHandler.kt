...
class IncomingPaymentHandler(val nodeParams: NodeParams, val db: PaymentsDb, private val remoteFundingRates: LiquidityAds.WillFundRates) {
                        ...
                        paymentPart.totalAmount != payment.totalAmount -> {
                            ...
                            val actions = payment.parts.map { part ->
                                ...
                                when (part) {
                                    ...
                                    is WillAddHtlcPart -> actionForWillAddHtlcFailure(privateKey, failureMsg, part.htlc)
                                }
                            }
                            ...
                            when {
                                willAddHtlcParts.isNotEmpty() -> when (val result = validateOnTheFlyFundingRate(willAddHtlcParts.map { it.amount }.sum(), currentFeerate)) {
                                    is Either.Left -> {
                                        ...
                                        val actions = payment.parts.map { part ->
                                            when (part) {
                                                is HtlcPart -> actionForFailureMessage(TemporaryNodeFailure, part.htlc)
                                                is WillAddHtlcPart -> actionForWillAddHtlcFailure(nodeParams.nodePrivateKey, TemporaryNodeFailure, part.htlc)
                                            }
                                        }
                                        ProcessAddResult.Rejected(actions, incomingPayment)
                                    }
                                    is Either.Right -> {
                                        ...
                                        val actions = listOf(OpenOrSplicePayment(payment.amountReceived, requestedAmount, fundingRate, incomingPayment.preimage))
                                    ...
                                    is Either.Left -> {
                                        ...
                                        val actions = htlcParts.map { part -> actionForFailureMessage(failure, part.htlc) }
                                        ProcessAddResult.Rejected(actions, incomingPayment)
                                    }
                                    is Either.Right -> {
                                        pending.remove(paymentPart.paymentHash)
    ...
    private fun validateOnTheFlyFundingRate(willAddHtlcAmount: MilliSatoshi, currentFeerate: FeeratePerKw): Either<LiquidityEvents.Rejected, Pair<Satoshi, LiquidityAds.FundingLease>> {
        return when (val liquidityPolicy = nodeParams.liquidityPolicy.value) {
            ...
            is LiquidityPolicy.Auto -> {
                ...
                val requestedAmount = willAddHtlcAmount.truncateToSatoshi() + additionalInboundLiquidity
                when (val fundingRate = remoteFundingRates.findLease(requestedAmount)) {
    ...
    private suspend fun validateFundingFee(parts: List<HtlcPart>): Either<ChannelException, LiquidityAds.FundingFee?> {
        return when (val fundingTxId = parts.map { it.htlc.fundingFee?.fundingTxId }.firstOrNull()) {
            is TxId -> {
                ...
                when (val lease = db.getInboundLiquidityPurchase(fundingTxId)?.lease) {
                    null -> Either.Left(UnexpectedLiquidityAdsFundingFee(channelId, fundingTxId))
                    else -> {
                        val paymentHashOk = when (val details = lease.paymentDetails) {
                        ...
                        val feeAmountOk = fundingFee <= lease.fees.total.toMilliSatoshi()
                        when {
                            paymentHashOk && feeAmountOk -> Either.Right(LiquidityAds.FundingFee(fundingFee, fundingTxId))
                            else -> Either.Left(InvalidLiquidityAdsFundingFee(channelId, fundingTxId, paymentHash, lease.fees.total, fundingFee))
                        }
                    }
                }
            }
