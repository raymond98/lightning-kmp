...
class InteractiveTxTestsCommon : LightningTestSuite() {
    ...
    fun `cannot contribute unusable or invalid inputs`() {
        ...
        run {
            ...
            val result = FundingContributions.create(channelKeys, swapInKeys, fundingParams, listOf(WalletState.Utxo(previousTx.txid, 0, 0, previousTx, WalletState.Companion.AddressMeta.Single))).left
        ...
        run {
            ...
            val result = FundingContributions.create(channelKeys, swapInKeys, fundingParams, listOf(WalletState.Utxo(previousTx.txid, 53, 0, previousTx, WalletState.Companion.AddressMeta.Single))).left
            assertNotNull(result)
        ...
        run {
            ...
            val result = FundingContributions.create(channelKeys, swapInKeys, fundingParams, listOf(WalletState.Utxo(previousTx.txid, 0, 0, previousTx, WalletState.Companion.AddressMeta.Single), WalletState.Utxo(previousTx.txid, 1, 0, previousTx, WalletState.Companion.AddressMeta.Single))).left
            assertNotNull(result)
    ...
    companion object {
        ...
        private fun createWallet(onChainKeys: KeyManager.SwapInOnChainKeys, amounts: List<Satoshi>, legacyAmounts: List<Satoshi> = listOf()): List<WalletState.Utxo> {
            return amounts.withIndex().map { amount ->
                ...
                WalletState.Utxo(parentTx.txid, 0, 0, parentTx, WalletState.Companion.AddressMeta.Derived(amount.index))
                ...
                val parentTx = Transaction(2, txIn, txOut, 0)
                WalletState.Utxo(parentTx.txid, 0, 0, parentTx, WalletState.Companion.AddressMeta.Single)
            }
        }
