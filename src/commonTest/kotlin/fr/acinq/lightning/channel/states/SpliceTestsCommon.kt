...
class SpliceTestsCommon : LightningTestSuite() {
    ...
    fun `splice to purchase inbound liquidity`() {
        ...
        val fundingRates = LiquidityAds.WillFundRates(
            fundingRates = listOf(LiquidityAds.FundingLease.Basic(100_000.sat, 500_000.sat, LiquidityAds.LeaseRate(0, 0.sat, 250 /* 2.5% */))),
            paymentTypes = setOf(LiquidityAds.PaymentType.FromChannelBalance),
        )
        val liquidityRequest = LiquidityAds.RequestFunds(200_000.sat, fundingRates.findLease(200_000.sat)!!, LiquidityAds.PaymentDetails.FromChannelBalance)
        ...
        assertEquals(spliceInit.requestFunds, liquidityRequest)
        ...
        run {
            val willFund = fundingRates.validateRequest(bob.staticParams.nodeParams.nodePrivateKey, fundingScript, cmd.feerate, spliceInit.requestFunds!!)?.willFund
        ...
        run {
            ...
            val willFund = fundingRates.validateRequest(bob.staticParams.nodeParams.nodePrivateKey, ByteVector("deadbeef"), cmd.feerate, spliceInit.requestFunds!!)?.willFund
            assertNotNull(willFund)
            val spliceAck = SpliceAck(alice.channelId, liquidityRequest.requestedAmount, 0.msat, defaultSpliceAck.fundingPubkey, willFund)
            val (alice2, actionsAlice2) = alice1.process(ChannelCommand.MessageReceived(spliceAck))
            assertIs<Normal>(alice2.state)
    ...
    fun `splice to purchase inbound liquidity -- not enough funds`() {
        ...
        val leaseRate = LiquidityAds.FundingLease.Basic(100_000.sat, 10_000_000.sat, LiquidityAds.LeaseRate(0, 1.sat, 100 /* 1% */))
        run {
            val liquidityRequest = LiquidityAds.RequestFunds(1_000_000.sat, leaseRate, LiquidityAds.PaymentDetails.FromChannelBalance)
        ...
        run {
            val liquidityRequest = LiquidityAds.RequestFunds(1_000_000.sat, leaseRate.copy(rate = leaseRate.rate.copy(leaseFeeBase = 0.sat)), LiquidityAds.PaymentDetails.FromChannelBalance)
            ...
            val aliceStfu = actionsAlice1.findOutgoingMessage<Stfu>()
            val (_, actionsBob2) = bob1.process(ChannelCommand.MessageReceived(aliceStfu))
            actionsBob2.hasOutgoingMessage<SpliceInit>().also { assertEquals(liquidityRequest, it.requestFunds) }
        }
        run {
            // When we don't  have enough funds in our channel balance, fees can be paid via future HTLCs.
            val liquidityRequest = LiquidityAds.RequestFunds(1_000_000.sat, leaseRate, LiquidityAds.PaymentDetails.FromFutureHtlc(listOf(randomBytes32())))
            assertEquals(10_001.sat, liquidityRequest.fees(FeeratePerKw(1000.sat)).total)
            val cmd = ChannelCommand.Commitment.Splice.Request(CompletableDeferred(), null, null, liquidityRequest, FeeratePerKw(1000.sat), listOf())
            val (bob1, actionsBob1) = bob.process(cmd)
            val bobStfu = actionsBob1.findOutgoingMessage<Stfu>()
