...
class SpliceTestsCommon : LightningTestSuite() {
    ...
    private fun doSplice(alice: LNChannel<Normal>, bob: LNChannel<Normal>, cmd: Command.Splice.Request): Pair<LNChannel<Normal>, LNChannel<Normal>> {
        ...
        val (alice6, actionsAlice6) = alice5.process(ChannelCommand.MessageReceived(actionsBob5.findOutgoingMessage<CommitSig>()))
        val (bob7, _) = bob6.process(ChannelCommand.MessageReceived(actionsAlice6.findOutgoingMessage<TxSignatures>()))
        val (alice7, _) = alice6.process(ChannelCommand.MessageReceived(actionsBob6.findOutgoingMessage<TxSignatures>()))
