...
class ShutdownTestsCommon : LightningTestSuite() {
    ...
    fun `recv CMD_ADD_HTLC`() {
        ...
        assertEquals(bob1.commitments.params.channelFeatures, ChannelFeatures(setOf(Feature.StaticRemoteKey, Feature.AnchorOutputs)))
    }
    ...
    fun `recv UpdateFailMalformedHtlc -- invalid failure_code`() {
        ...
        assertTrue(actions.contains(ChannelAction.Blockchain.PublishTx(alice.commitments.latest.localCommit.publishableTxs.commitTx.tx, ChannelAction.Blockchain.PublishTx.Type.CommitTx)))
