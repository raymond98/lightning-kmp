...
class PeerTest : LightningTestSuite() {
    ...
    fun `swap funds into a channel`() = runSuspendTest {
        ...
        bob.send(OpenOrSpliceChannel(walletBob))
        ...
        assertEquals(open.requestFunds?.requestedAmount, 100_000.sat) // we always request funds from the remote, because we ask them to pay the commit tx fees
    ...
    fun `reject swap-in -- fee too high`() = runSuspendTest {
        ...
        val rejected = bob.nodeParams.nodeEvents.first()
